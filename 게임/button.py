...
class btn01:
    ...
    # [ 이미지 이름, 이미지가 출력될 위치(좌하단), 이미지의 사이즈, 이미지 처리 이벤트, 이미지를 n개로 나눌지에 해당하는 n ]을 인자로 받는다.
    def __init__(self, image, x, y, sx, sy, event, bSelect, bleft, n):
        # 버튼의 이미지
        self.Image = gfw.image.load(image)
    ...
    def draw(self):
        self.Image.clip_draw_to_origin(self.Image.w // self.image_divide * self.dx, 0, self.Image.w // self.image_divide, self.Image.h, self.x + self.alpha, self.y, self.sx, self.sy)
